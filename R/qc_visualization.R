...
create_read_count_matrix <- function(all_df, filtered_df = NULL, group_csv = NULL, crf_col = "crf", category_col = "category", by = "-") {
  # Extract CRFs
  ...
  # Mark pairs absent from all_df as NA (no data at all)
  all_pair_set <- new.env(hash = TRUE, parent = emptyenv())
  for (pair in as.character(all_df$pair)) {
    all_pair_set[[pair]] <- TRUE
    parts <- strsplit(pair, by, fixed = TRUE)[[1]]
    all_pair_set[[paste(rev(parts), collapse = by)]] <- TRUE
  }
  lower_tri_idx <- which(lower.tri(mat, diag = FALSE), arr.ind = TRUE)
  lower_pair_names <- paste(crfs[lower_tri_idx[, 1]], crfs[lower_tri_idx[, 2]], sep = by)
  is_absent <- !sapply(lower_pair_names, function(p) exists(p, envir = all_pair_set))
  mat[lower_tri_idx[is_absent, , drop = FALSE]] <- NA

  # Mark non-QC-passed pairs as NA (only when filtered_df is provided)
...
#' @param filtered_read_count_path Path to filtered_qc.tsv generated by qc_count.sh. Optional; if NULL, all pairs are shown without grey filtering.
...
qc_visualization <- function(all_read_count_path, filtered_read_count_path = NULL, out_dir = "./", split_pair_by = "-", group_csv = NULL, crf_col = "crf", category_col = "category") {
  ...
  if (!is.null(filtered_read_count_path)) {
